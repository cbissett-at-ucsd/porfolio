...
namespace ECE141 {
  ...
  bool Tokenizer::next(int anOffset) {
   //   std::cout << "next (in tokenizer)" << index << std::endl;
  ...
  Token& Tokenizer::current() {
      //std::cout << "current (in tokenizer)" << index << std::endl;
    return tokens[index];
  }
  ...
  size_t Tokenizer::index_cur(){
      return index;
  }

  bool Tokenizer::skipToSemi(){
      while (remaining()) {
          if (current().data == ";") {
              next();
              return true;
  ...
  bool Tokenizer::skipToChar(std::string aChar){
      while (remaining()) {
          if (current().data == aChar) {
              next();
              return true;
          }
          else {
