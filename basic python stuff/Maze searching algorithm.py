...
def get_new_destination(current_position, unexplored_areas):
    ...
    norms=[]
    i=0
    for xy in unexplored_areas:
        diff_x=current_position[0]-xy[0]
        diff_y=current_position[1]-xy[1]
        a_norm=np.sqrt(diff_x*diff_x+diff_y*diff_y)
        norms.append(a_norm)
    min_pos=norms.index(min(norms))
    return unexplored_areas[min_pos]
...
def update_explore_map(dest, route, explore_map, planned, unmapped):
    ...
    '''
    for loc in route:
        if explore_map[loc[0]][loc[1]]<planned:
            explore_map[loc[0]][loc[1]]=planned
    if explore_map[dest[0]][dest[1]]<planned:
...
def update_position(curPos, route, dest, explore_map, mapped):
    ...
    if curPos[0]==route[0][0] and curPos[1]==route[0][1]:
        explore_map[curPos[0]][curPos[1]]=mapped
        route=route[1:]
    #1
    curPos=[route[0][0],route[0][1]]
    #2
    explore_map[curPos[0]][curPos[1]]=mapped
    #3
